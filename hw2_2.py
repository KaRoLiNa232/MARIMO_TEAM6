...
def _(df_1, pd):
    ...
    df_2 = df_2[
        (df_2["kitchen_area"] >= 2)
        & (df_2["kitchen_area"] <= 0.5 * df_2["area"])
    ]
    ...
    df_2["datetime_str"] = df_2["date"].astype(str) + " " + df_2["time"].astype(
        str
    )
...
def _():
    ...
    region_map_second = dict(
        sorted(region_map_second.items(), key=lambda item: item[1])
    )
